...
class cluster_tools::config {
  ...
  if $facts['host_realm'] {
    ...
    notice(sprintf('>> %-20s: ACCESS DEBUG | pp_project = %-20s | pp_cluster = %-20s | group_identifier = %-20s', $certname, $::cluster_tools::pp_project, $::cluster_tools::pp_cluster,$group_identifier))
